...
class PrestamosApp(ttk.Frame):
    def __init__(self, parent, role):
        ...
        search_frame = ttk.LabelFrame(self, text="Buscar Préstamos", padding=(10, 5))
        ...
        # Treeview para mostrar los préstamos
        columns = ('idprestamo', 'idlibro', 'idusuario', 'fecha_prestamo', 'fecha_devolucion')
        self.tree = ttk.Treeview(self, columns=columns, show='headings', height=15)
        for col in columns:
            self.tree.heading(col, text=col.replace('_', ' ').title())
            self.tree.column(col, anchor=tk.CENTER)
        self.tree.pack(expand=True, fill='both', pady=(0, 10), padx=10)

        if self.role != 'bibliotecario':
            self.add_button = ttk.Button(self, text="Agregar Préstamo", command=self.open_add_window)
    ...
    def open_add_window(self):
        ...
        form_frame = ttk.LabelFrame(add_window, text="Nuevo Préstamo", padding=(10, 5))
        form_frame.pack(pady=10, padx=10)

        ttk.Label(form_frame, text="ID Libro:").grid(row=0, column=0, padx=5, pady=5, sticky='e')
        idlibro_entry = ttk.Entry(form_frame)
        idlibro_entry.grid(row=0, column=1, padx=5, pady=5, sticky='w')

        ttk.Label(form_frame, text="ID Usuario:").grid(row=1, column=0, padx=5, pady=5, sticky='e')
        idusuario_entry = ttk.Entry(form_frame)
        idusuario_entry.grid(row=1, column=1, padx=5, pady=5, sticky='w')

        ttk.Label(form_frame, text="Fecha Préstamo (YYYY-MM-DD):").grid(row=2, column=0, padx=5, pady=5, sticky='e')
        fecha_prestamo_entry = ttk.Entry(form_frame)
        fecha_prestamo_entry.grid(row=2, column=1, padx=5, pady=5, sticky='w')

        ttk.Label(form_frame, text="Fecha Devolución (YYYY-MM-DD):").grid(row=3, column=0, padx=5, pady=5, sticky='e')
        fecha_devolucion_entry = ttk.Entry(form_frame)
        fecha_devolucion_entry.grid(row=3, column=1, padx=5, pady=5, sticky='w')
    ...
    def add_prestamo(self, idlibro, idusuario, fecha_prestamo, fecha_devolucion):
    ...
    def open_edit_window(self):
        ...
        form_frame = ttk.LabelFrame(edit_window, text="Editar Préstamo", padding=(10, 5))
        form_frame.pack(pady=10, padx=10)
...
if __name__ == "__main__":
    ...
    role = 'usuario'  # Definir un rol para probar
    app = PrestamosApp(root, role)
